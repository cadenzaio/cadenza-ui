...
async function getSignal(name: string, serviceName: string) {
  const query = `
    ...
    WHERE name = $1 AND service_name = $2 AND is_meta = true
    ORDER BY name ASC
  `;
...
async function getPreviousTasksForSignal(signalName: string, serviceName: string) {
  const query = `
    ...
    WHERE tsm.signal_name = $1 AND tsm.service_name = $2 AND is_meta = true
    ORDER BY t.name ASC
  `;
...
async function getNextTasksForSignal(signalName: string, serviceName: string) {
  const query = `
    ...
    JOIN task t ON tsm.task_name = t.name
    WHERE tsm.signal_name = $1 AND tsm.signal_service_name = $2 AND is_meta = true
    ORDER BY t.name ASC
  `;
